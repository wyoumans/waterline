 ...
 * Add Auto Attributes
 *
 * Mixes in primaryKey, created, modified attributes
...
Schema.prototype.addAutoAttributes = function() {
  ...
  // extend definition with autoModified and autoCreated
  var now = { type: 'datetime', 'default': 'NOW' };
  if(this.context.autoCreated && !this.schema.created) this.schema.created = now;
  if(this.context.autoModified && !this.schema.modified) this.schema.modified = now;
};
